...
extrai_ilhas_linha(Num_Linha, Linha_puzzle, Ilhas) :-
    extrai_ilhas_linha(Num_Linha, Linha_puzzle, 1, [], Ilhas), !.

extrai_ilhas_linha(_, [], _, Ilhas_Aux, Ilhas_Aux).


extrai_ilhas_linha(Num_Linha, [P|R], Ind, Ilhas_Aux, Ilhas) :-
    ( \==(P, 0) -> append(Ilhas_Aux, [ilha(P, (Num_Linha, Ind))], Ilhas_Aux2);
    Ilhas_Aux2 = Ilhas_Aux), 
    Index is Ind + 1,
    extrai_ilhas_linha(Num_Linha, R, Index, Ilhas_Aux2, Ilhas).
...
ilhas([P|R], Num_Linha, Ilhas_Aux, Ilhas) :-   
    extrai_ilhas_linha(Num_Linha, P, Ilhas_L), 
    append(Ilhas_Aux, Ilhas_L, Ilhas_Aux2), 
    % juntar a lista de ilhas da linha Num_Linha ah lista de ilhas ja existente
    Num_Linha_2 is Num_Linha + 1,
    ilhas(R, Num_Linha_2, Ilhas_Aux2, Ilhas).
...
% Da as posicoes na mesma linha/coluna, entre as colunas/linhas CL_1 e CL_2.
posicoes_entre_aux(CL_1, CL_2, CL_Comum, Mesma_LC, Posicoes) :-
    findall(Entre, 
        (between(CL_1, CL_2, Entre), Entre \== CL_1, Entre \== CL_2), 
        Lst_Entre_CL),
    (Mesma_LC == 'Mesma_Linha' -> 
        findall(P, (P = (CL_Comum, Entre), member(Entre, Lst_Entre_CL)), Posicoes);         
    % posicoes na mesma linha, entre as colunas de Pos1 e Pos2
    findall(P, (P = (Entre, CL_Comum), member(Entre, Lst_Entre_CL)), Posicoes)). 
...
posicoes_entre(Pos1, Pos2, Posicoes) :-
    ...
    % nao teriam posicoes entre elas
    (L1 == L2; C1 == C2),  
    ( L1 == L2 -> (C1 < C2 -> posicoes_entre_aux(C1, C2, L1, 'Mesma_Linha', Posicoes);   
    posicoes_entre_aux(C2, C1, L1, 'Mesma_Linha', Posicoes));    % se C1 > C2
    % se L1 \== L2, entao C1 == C2 necessariamente
    (L1 < L2 -> posicoes_entre_aux(L1, L2, C1, 'Mesma_Coluna', Posicoes);  
    (posicoes_entre_aux(L2, L1, C1, 'Mesma_Coluna', Posicoes)))).    % se L1 > L2
...
% predicado auxiliar de caminho_livre/5
% Informa se alguma posicao entre 2 ilhas coincide com uma posicao onde ha uma ponte.
caminho_livre_aux([], _). 
...
% Predicado auxiliar de junta_pontes/5
% Adiciona as pontes de Lst_pontes a uma entrada, se esta for a entrada da Ilha1 
% ou Ilha2 (ilhas unidas pelas pontes de Lst_pontes).
